...
import { DataType, useAuthentication } from '@/context/UserContext'


export default function Output() {
    // const [selectedDatatype, setselectedDatatype] = useState(DataType[0])
  const {output,setoutput}=useAuthentication()
    console.log(output)
  return (
    <div className=" top-16 w-72">
      <Listbox value={output} onChange={setoutput}>
                    ...
                    <>
                      <span
                        className={`block truncate ${
                            output ? 'font-medium' : 'font-normal'
                        }`}
                      >
                        {datatype.name}
                      </span>
